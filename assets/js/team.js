...
function renderEventMeta(){
  ...
  if($("eventStatus")) $("eventStatus").textContent = ev.status || "Draft";
  if($("viewDate")) $("viewDate").textContent = ev.date || "—";
  if($("viewVenue")) $("viewVenue").textContent = ev.venue || "—";
  if($("viewCity")) $("viewCity").textContent = ev.city || ev.locationText || "—";
  if($("designHeadline")) $("designHeadline").textContent = ev.design?.headline || "—";
  if($("designEmail")) $("designEmail").textContent = ev.design?.emailText || "—";
  if($("designPrimary")) $("designPrimary").textContent = ev.design?.primary || "—";
  if($("designAccent")) $("designAccent").textContent = ev.design?.accent || "—";
  if($("designFont")) $("designFont").textContent = ev.design?.fontFamily || "—";
